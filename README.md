# 🚀 Alembic + SQLAlchemy (async) + PostgreSQL

## Руководство по работе с миграциями
Данный проект использует **Alembic** для управления миграциями базы данных в связке с
**SQLAlchemy (async) и PostgreSQL.**

Документ описывает порядок инициализации Alembic, настройки под асинхронное подключение, создания и применения миграций, а также важные нюансы работы с ENUM и ForeignKey.

## 🚀 Стек технологий
- Python 3.13
- PostgreSQL
- SQLAlchemy 2.0+ (async)
- Alembic
- asyncpg

## 📦 Установка зависимостей
`pip install sqlalchemy alembic asyncpg psycopg`

## ⚙️ Инициализация Alembic (async)
В корне проекта выполните:

`alembic init -t async migration`

Будет создана директория **migration/** с шаблонами для асинхронной работы SQLAlchemy.

## 🧱 Создание миграций
После описания моделей:

`alembic revision --autogenerate -m "Initial revision"`

Alembic создаст файл в **migration/versions/**

## 🚀 Применение миграций
`alembic upgrade head`

## ⏪ Откат миграций
Откат на одну версию назад:

`alembic downgrade -1`

Откат к конкретной версии:

`alembic downgrade <revision_id>`

# ⚠️ ВАЖНО: Работа с ENUM
## ✅ Всегда указывать create_type=False
```python
sa.Column(
    "status",
    sa.Enum("active", "inactive", name="user_status",create_type=False), nullable=False
)

Без этого Alembic может падать при повторных миграциях. 

## 🧹 Корректное удаление ENUM при downgrade
В downgrade():
```python
def downgrade():
    op.drop_table("users")
    op.execute("DROP TYPE IF EXISTS user_status")